...
Instead, we propose comparing the measurements by weighting speaker endorsements based on *need probabilities*, that is, the probability with which a speaker would have to refer to a kind with a particular level of prevalence [@Kemp2018].
...
For the asymmetry analysis, the mean implied prevalence $\mathbb{E}[r_{implied}] = \sum_{r} r \cdot \hat{P}_{implied}(r \mid u)$ is compared to the expected endorsement prevalence $\mathbb{E}[r_{endorsed}] = \sum_{r} p\cdot \hat{P}_{endorsed}(r \mid u)$, where $\hat{P}_{endorsed}(r \mid u) \propto \hat{P}_{endorsed}(u \mid r) \cdot P(r)$.
By taking the mean of the prevalences at which a participant endorsed the generic, @Cimpian2010's analysis realizes this calculation by assuming a uniform prior on prevalences $P(r)$.
...
For example, let's say participants rate the item "Lorches are afraid of loud noises" as true at the five prevalence levels (10, 30, 50, 70, 90%) with proportions $[0.3, 0.3, 0.8, 0.9, 0.9]$ (e.g., 30% of participants endorse the statement when 10% or 30% of lorches live in trees, 90% of participants endorse the statement when 70% or 90% live in trees, etc.).
...
But by assuming a uniform prior on prevalence $P(r)$, @Cimpian2010 assume that speakers would encounter kinds with any prevalence level equally as often. 
To more accurate model the situations with which a speaker would encounter kinds with various levels of prevalence for a property, we propose using a prior on prevalence that encodes *need probabilities*, that is, the probability that a speaker would need to communicate about a kind with that level of prevalence.
...
Using these probabilities as $P(r)$, we get the following:
$$
[0.3 \times 0.18, 0.3 \times 0.1, 0.8 \times 0.15, 0.9 \times 0.15, 0.9 \times 0.42] \\
\propto [0.06, 0.05, 0.18, 0.2, 0.5]
$$
Using these need probabilities, higher prevalence levels have higher posterior probabilities of being endorsed because they are more likely to occur in the environment. 
The expected value of this distribution, the average endorsed prevalence, is 70%.
Thus, the apparent asymmetry is dramatically reduced when we take into the prior probability that a speaker would need to refer to such a prevalence level.
