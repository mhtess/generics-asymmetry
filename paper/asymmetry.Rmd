...
These two intuitions---generics have flexible truth conditions and are interpreted strongly---leads to a potential asymmetry. 
...
This is the essence of the asymmetry.
The asymmetry is worrisome: Generics are a primary vehicle for conveying stereotypes [@GelmanEtAl2004]. 
The decolage between endorsements and interpretations means that listeners might draw a too strong generalization than the speaker intended.
...
Here, we investigate the source of the asymmetry that was reported by @Cimpian2010, discovering that is a statistical artifact that results from the transformations employed to compare the two data sources (truth conditions and implied prevalence).
...
We use our computational model previously fit to the implied prevalence data earlier, then, to generate predictions for the average endorsement prevalence. 
...
Because of the mathematical properties of *average endorsed prevalence*, the expected variability across items is very small: All model predictions range between 0.52 and 0.73 (total range = 0.21). 
Note that the same items have average implied prevalence ratings that range between 0.37 and 0.95 (total range 0.58).
Therefore, it is likely that the averaged endorsed prevalence is a much less reliable measurement. 
...
The results were extremely comparable using for the more relaxed exclusion criteria.
