...
As indicated by the earlier analyses, the uniform need probabilities do not provide any quantitative variation to distingiush among endorsement patterns for different generics, so the resulting asymmetries are entirely driven by differences in the average implied prevalence (Figure\ \@ref(fig:aepAipScatter)A).
When the need probabilities reflect the statistics of the environment, however, asymmetries result both by differnces in endorsements and in implied prevalence (Figure\ \@ref(fig:aepAipScatter)B).
The result, then, the majority of properties still show an asymmetry wherein implied prevalence is greater than endorsed prevalence (points above the $y=x$ line) provides more compelling evidence that generics do result in *evidence exaggeration*, even controlling for the environmental statistics of use.

```{r aepScatter, fig.width = 10, fig.asp = 0.5, fig.cap="Here is a caption.", cache = F}
...
```{r aepAipScatter, fig.width = 7, fig.asp = 0.5, fig.cap="Here is a caption.", cache=F}
df.end.1.aep.aip %>%
...
Generics provide interesting flexiblity, however, as they can be endorsed or asserted even when the property prevalence is very low (see Chapter 3 for a detailed investigation). 
...
We propose an alterantive method, based on that of *need probabilities*, which take into account the distribution of situations in which speakers would have to refer to kinds with various levels of property prevalence. 
...
Our computational model provides only a modest fit to the average endorsement pattern, in comparison in particular to an alternative model that endorses generic statements in a property-invariant way (assigining the same endorsement patterns to all properties). 
