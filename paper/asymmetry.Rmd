...
Now that we have made explicit the mathematical relationship between implied and endorsed prevalence, we can apply the same analysis on a by-item, as opposed to a by-participant, basis.^[
  If one wanted to calculate average endorsed prevalence on a participant-wise basis in addition to an item-wise basis, each participant must make a judgment at every prevalence level for each item. 
  This is perhaps pragmatically strange (though the task is already a bit pragmatically strange) and @Cimpian2010 does not do this. 
  Instead, he calculates average endorsed prevalence at the participant level and aggregates at the level of *property type*  in order to compare domain differences (e.g., treating *purple feathers* and *blue scales* as the same kind of property). 
]
...
To treat turn proportions into a probability distribution $P_{endorsed}(p \mid u)$, we would divide each by the sum of all endorsement proportions (e.g., $\frac{0.2}{0.2 + 0.2 + 0.6 + 0.9 + 0.9} = 0.07$, $\frac{0.9}{0.2 + 0.2 + 0.6 + 0.9 + 0.9} = 0.32$), yielding the following distribution [0.07, 0.07, 0.21, 0.32, 0.32].
...
To compare to the theoretical analysis for *most*, let us assume that $P(p)$ is uniform; that is, $P(p) \propto 1$, .
...
therefore:  $L(p \mid u) = 2p$. The expected value of the implied prevalence is then given by:
...
So the average implied prevalence given a generic statement assuming a uniform prior over prevalence is 66%.
What about the average endorsed prevalence?
<!-- Again, assuming a uniform prior: -->
\begin{align}
S(u \mid p) \propto L(p \mid u) = (2p)^\alpha
\end{align}
Assume $\alpha = 1$. 
Then we just have $S(u \mid p) = 2p$. 
Computing average endorsed prevalence via Bayes' rule, we get $S_{endorsed}(p \mid u) \propto 2p \times P(p) = 2p \times 1 = 2p$, the expectation of which will also be $\frac{2}{3}$.
