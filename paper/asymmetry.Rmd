...
The fact that listeners infer higher prevalence than speakers had in mind would explain why such stereotypes get propagated, even when the actual prevalence may be low. 

The asymmetry is puzzling from a theoretical perspective as well.
...
These proportions are treated as an *unnormalized* likelihood distribution $\hat{P}_{endorsed}(u \mid r)$, multiplied by a uniform prior $P(r) = [1, 1, 1, 1, 1]$ to arrive at an unnormalized posterior of interest: $\hat{P}_{endorsed}(r \mid u)$. 
With a uniform prior on $P(r)$, the posterior $\hat{P}_{endorsed}(r \mid u)$ is the same as the likelihood $\hat{P}_{endorsed}(u \mid r)$ (up to normalization).
...
This will of course have problems for addressing questions of societal concern (e.g., propagation of stereotypes), but it is more realistic than assuming a uniform distribution. 
...
All participants were self-reported native English speakers and none completed the analogous implied prevalence task (Ch. 2 Expt. 1B).
...
```{r figCbgResults, fig.width=11, fig.asp = 0.75, fig.cap="Replication of @Cimpian2010 Expt. 1. A: Endorsements for novel generic sentences in Expt. 1. B: Prevalence (need) priors from Ch. 2, Expt. 1. C: The asymmetry between endorsed prevalence and implied prevalence computed using Cimpian's method (uniform priors). D: Aymmetries computed using need priors (shown in C)."}
...
Figure\ \@ref(fig:cbgAidAedByItem) shows the results. 
...
We next examine the endorsement data by computing average endorsed prevalence assuming a need probability distribution that follows the environmental statistics of the property, operationalized using the prevalence prior elicited for these same items in Ch. 2 (Expt. 2a). 
...
For example, the property *is afraid of loud noises* has a 0.43 probability of occurring around 90% prevalence within a kind (averaging between 80-100% prevalence) whereas the property *carries malaria* only has a 0.09 probability of occurring with that same prevalence; the endorsement ratings are thus weighted to reflect these environmental statistics, providing a more accurate measure of the average prevalence at which we would expect speakers to use the generics.
...
df.end.1.aep_need.stats <- df.end.1.aep_need %>% ungroup() %>%
  summarize( r = cor(aep_mean, MAP),
             mse = mean((aep_mean-MAP) ^ 2),
             n = n())

df.end.1.aep_need.prior.stats <- df.end.1.aep_need.prior %>% ungroup() %>%
  summarize( r = cor(need_endorsed_prevalence, uniformLikelihood_aep),
             mse = mean((need_endorsed_prevalence-uniformLikelihood_aep) ^ 2),
             n = n())

df.end.1.aep_need.invariant.stats <- df.end.1.aep_need.invariant %>% ungroup() %>%
  summarize( r = cor(need_endorsed_prevalence, prevLikelihood_aep),
             mse = mean((need_endorsed_prevalence-prevLikelihood_aep) ^ 2),
             n = n())
```
...
We find that the generics endorsement model shows a strong correlation to the average endorsed prevalence ($r(`r df.end.1.aep_need.stats[[1,"n"]]`) = `r round(df.end.1.aep_need.stats[[1,"r"]], 3)`$, MSE $=`r df.end.1.aep_need.stats[[1,"mse"]]`$; Figure\ \@ref(fig:aepScatter)A).
To better understand the contribution of the need probabilities to the resulting average endorsed prevalence, we ran control models that only used the need probabilities (i.e., if the model endorsed the generic equally as often for all prevalence levels and all properties) and that used the need probabilities and a context-invariant endorsement model (i.e., if the model endorsed the generic proportional to the prevalence level, but with no differentiation among the properties).
These control models also show a strong correlation to the data (prior only: $r(`r df.end.1.aep_need.prior.stats[[1,"n"]]`) = `r round(df.end.1.aep_need.prior.stats[[1,"r"]], 3)`$, MSE $=`r df.end.1.aep_need.prior.stats[[1,"mse"]]`$; Figure\ \@ref(fig:aepScatter)B, and context-invariant endorsement model: $r(`r df.end.1.aep_need.invariant.stats[[1,"n"]]`) = `r round(df.end.1.aep_need.invariant.stats[[1,"r"]], 3)`$, MSE $=`r df.end.1.aep_need.invariant.stats[[1,"mse"]]`$; Figure\ \@ref(fig:aepScatter)C), though the prior only model consistently *underpredicts* the average endorsed prevalence while the context-invariant model consistently *overpredicts* the average endorsed prevalence.

Finally, we look at the asymmetry between endorsed prevalence and implied prevalence (taken from Ch. 2, Expt. 2a), both when average endorsed prevalence is computed assuming uniform need probabilities [as done by @Cimpian2010] and assuming environmental statistics for the need probabilities. 
As indicated by the earlier analyses, the uniform need probabilities do not provide any quantitative variation to distinguish among endorsement patterns for different generics, so the resulting asymmetries are entirely driven by differences in the average implied prevalence (Figure\ \@ref(fig:aepAipScatter)A left).
When the need probabilities reflect the statistics of the environment, however, asymmetries result both by differences in endorsements and in implied prevalence (Figure\ \@ref(fig:aepAipScatter)A right).
The result, then, the majority of properties still show an asymmetry wherein implied prevalence is greater than endorsed prevalence (points above the $y=x$ line; Figure\ \@ref(fig:aepAipScatter)G) provides more compelling evidence that generics do result in *evidence exaggeration*, even controlling for the environmental statistics of use.

Due to unreliability of the endorsement data and the exploratory nature of this approach, it is hard to draw generalizations from the properties that do or do not exhibit an asymmetry.
One observation, however, is that many of the properties that do not exhibit *evidence exaggeration* are properties of animals interacting with the human world (e.g., *get addicted to nicotine*, *drink soda*, *perform in the circus*).
These may be cases where the property can be attributed to an extrinsic cause (in this case, human beings actively enabling the property). 
Further investigation should investigate this possibility.

```{r aepScatter, fig.width = 10, fig.asp = 0.5, fig.cap="Empirical average endorsed prevalence calculated using a need probability distribution based on the environmental statistics (prevalence prior) vs. model predictions. A: Generic endorsement model. B: A model that always endorses the generic the same regardless of the prevalence level or property. C: A model that endorses the generic as a function of a prevalence but without respect to property.", cache = F}
fig.aep_need.modelscatter <- df.end.1.aep_need %>%
...
fig.aep_need.priorscatter <- df.end.1.aep_need.prior %>%
...
fig.aep_need.invariantscatter <-  df.end.1.aep_need.invariant %>%
  ggplot(., aes( y = need_endorsed_prevalence,
...
```{r aepAipScatter, fig.width = 13, fig.asp = 0.35, fig.cap="Asymmetry between implied prevalence and endorsed prevalence. A: Implied prevalence vs. average endorsed prevalence calculated using a uniform need probability (Cimpian et al., 2010 method) and a need probability distribution based on the environmental statistics (prevalence prior). B: Environmental need probability calculated asymmetries. ", cache=F}
fig.aep.aip.scatters <- df.end.1.aep.aip %>%
...
df.end.1.aep.aip.asym <- df.end.1.aep.aip %>% 
  filter(src == "aep") %>% 
  mutate(asym = 100*mean - aep_mean)

df.end.1.aep_unif.aip.asym <- df.end.1.aep.aip %>% 
  filter(src == "aep_unif") %>% 
  mutate(asym = 100*mean - aep_mean)

fig.asym.property <- df.end.1.aep.aip.asym %>%
  ungroup() %>%
  mutate(property = factor(property, levels = property[order(-asym)])) %>%
  ggplot(., aes (x = property, y = asym))+
  geom_col(position = position_dodge())+
  coord_flip()+
  xlab("")+
  ylab("Asymmetry\n(Implied - Endorsed prevalence)")

cowplot::plot_grid(
  fig.aep.aip.scatters,
  fig.asym.property,
  nrow = 1,
  labels = c("A", "B"),
  rel_widths = c(1.2, 1)
...
In this chapter, we examined the relationship between the context-sensitivity of generic endorsements in relationship to that of generic interpretation. 
This relationship is broadly interesting because much of our conceptual understanding of the world is transmitted through generic statements. 
Generics provide interesting flexibility, however, as they can be endorsed or asserted even when the property prevalence is very low (see Chapter 3 for a detailed investigation). 
Generics also have context-sensitive interpretations (see Chapter 2). 
If, however, generics are endorsed overall at lower prevalence levels than they are interpreted, this would lead to an exaggeration of evidence. 

Previously, @Cimpian2010 found that there was an asymmetry such that generics are interpreted to apply more strongly than their corresponding endorsement patterns would suggest.
The method used in that study, we show here, was systematically biased to reduce the variance in the endorsement measure, thus making the asymmetry entirely driven by differential interpretations.
We propose an alternative method, based on that of *need probabilities*, which take into account the distribution of situations in which speakers would have to refer to kinds with various levels of property prevalence. 
We found that this measure of average endorsed prevalence, based on a proxy for environmental statistics, exhibits much more variability in items that we know to have high variability in implied prevalence. 
Using our method, we find many items do show an asymmetry, but one that is driven by both the implied prevalence and the endorsement patterns.

Our computational model provides only a modest fit to the average endorsement pattern, in comparison in particular to an alternative model that endorses generic statements in a property-invariant way (assigning the same endorsement patterns to all properties). 
The aim of the computational model, however, is to predict the endorsement data at level of a particular property and prevalence level (i.e., predict the proportion of participants who endorse the statement "Lorches have brown fur", when 10%, 30%, etc. have brown fur), not predict the average endorsement responses. 
Further, the data at the level of individual properties and prevalences is not itself very reliable. 
Using the Spearman prediction formula, the highest expected correlation one might expect to get is around 0.5, but this estimate also has very high variance (ranging form 0.08 - 0.74). 
Thus, aiming to fit a model to these highly noisy data is unlikely to produce reliable predictions. 
It is unnecessary to have equal numbers of participants rate all prevalence and property combinations, however, as the need probability distribution downweights certain conditions. 
It might be desirable, instead, to oversample the higher probability property--prevalence combinations (e.g., 10% have malaria, 90% have brown fur) rather than uniformly distribute participants to property--prevalence conditions.
