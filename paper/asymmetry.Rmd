...
In Chapter 3, we saw how generic statements can be intuitively true under a wide range of statistical conditions. 
...
With the exception of how the analysis handles participants who never endorse the statement, @Cimpian2010's analysis is an application of Bayes' rule.
...
@Cimpian2010's analysis realizes this calculation assuming a uniform prior on prevalences $P(p)$.
...
For example, let's say participants rate the item "Lorches live in trees" as true at the five prevalence levels (10, 30, 50, 70, 90%) with proportions $[0.2, 0.2, 0.6, 0.9, 0.9]$ (e.g., 20% of participants endorse the statement when 10% or 30% of lorches live in trees, 90% of participants endorse the statement when 70% or 90% live in trees, etc.).
We treat the proportions as an *unnormalized* probability distribution $P_{endorsed}(p \mid u)$ and normalize by dividing each by the sum of all endorsement proportions (e.g., $\frac{0.2}{0.2 + 0.2 + 0.6 + 0.9 + 0.9} = 0.07$, $\frac{0.9}{0.2 + 0.2 + 0.6 + 0.9 + 0.9} = 0.32$), yielding the following distribution: [0.07, 0.07, 0.21, 0.32, 0.32].
We then take the expectation (i.e., the mean) of this distribution with respect to the prevalence level associated with each probability: *average endorsed prevalence* $= 0.07 \times 10\% + 0.07 \times 30\% + 0.21 \times 50\% + 0.32 \times 70\% + 0.32 \times 90\% = 65\%$.
The *average implied prevalence* is the mean of participants responses to the question: "How many lorches live in trees?" after learning that "Lorches live in trees" ($\sim 90\%$, from Ch. 2 Expt. 2). 
Thus, in this example, there is an asymmetry between the average endorsed prevalence ($65\%$) and average implied prevalence ($\sim 90\%$).
...
From Chapters 2 and 3, we know that the following are good models for $P_{implied}(p \mid generic)$ and $P_{endorsed}(generic \mid p)$:
...
To compare to the theoretical analysis for *most*, let us assume that the prevalence prior is uniform; that is, $P(p) \propto 1$, .
That makes, $L(p \mid u) \propto p$.
To compute the expectation of $L(p \mid u)$, we must first normalize the distribution:
...
\begin{align}
S(u \mid p) \propto L(p \mid u) = (2p)^\alpha
\end{align}
Assume $\alpha = 1$. 
Then $S(u \mid p) = 2p$. 
Following @Cimpian2010, computing average endorsed prevalence via Bayes' rule assuming a uniform prior: 
$$S_{endorsed}(p \mid u) \propto 2p \times P(p) = 2p \times 1 = 2p$$
...
Thus, the asymmetry for the average implied prevalence and average endorsed prevalence does not result from the semantics of generics itself. We next explore through simulation what happens when we do not assume a uniform prior on prevalence. 
Figure\ \@ref(fig:asymmetrySimulations) shows the predicted asymmetries for three different prevalence priors. 
We see that the asymmetry is driven by the interpretation (Figure\ \@ref(fig:asymmetrySimulations)B which in turn depends heavily on the prevalence prior (Figure\ \@ref(fig:asymmetrySimulations)A). 
The endorsement model, though sensitive to the prevalence prior in exactly the same way  (Figure\ \@ref(fig:asymmetrySimulations)C), is not directly compared to interpretations: Instead, we compute *average endorsed prevalence* which essentially combines the endorsement ratings with a uniform distribution (i.e., the assumed uniform prior on $p$ described above). 
This has the effect of decreasing the variance of the average endorsed prevalence, thus making average endorsed prevalencee less sensitive to the prevalence prior (Figure\ \@ref(fig:asymmetrySimulations)D). 
...
```{r asymmetrySimulations, fig.width=11, fig.asp=0.54, cache=F, fig.cap="Model simulations. A: Three theoretically interesting prior distributions over prevalence. B: Generic interpretation (implied prevalence) distributions. C: Endorsement model predictions given different referent prevalence levels and different prevalence priors. D: Average endorsed prevalence (blue) vs. average implied prevalence (red). The predicted asymmetry depends upon the prevalence priors and is mostly driven the implied prevalence."}
...
```{r figCbgResults, fig.width=10.5, fig.asp = 0.35, fig.cap="Proportion of endorsements for novel generic sentences in Expt. 1 (Replication of @Cimpian2010 Expt. 1)."}
...
Figure\ \@ref(fig:figCbgResults)A show the proportion of endorsements at each level of prevalence for the four differet kinds  of properties.
To estimate the effects, we ran a Bayesian bernoulli mixed-effects regression model with fixed effects of prevalence (centered and scaled), property type and their interaction and random effects of intercept and property type by-participants and intercept by-item.
...
In addition, there is evidence for a small interaction between prevalence and property type for the body $\beta_{body \times prevalence} =$ `r format_regression_effects(rs.brm.endorse.prev.type.summary, "stim_prevalence:stim_typepart")` and gradable properties $\beta_{gradable \times prevalence} =$ `r format_regression_effects(rs.brm.endorse.prev.type.summary, "stim_prevalence:stim_typevague")` in comparison to the accidental properties: The slope appears flatter for the accidental bare plurals, owing to the fact they are endorsed less at high prevalence levels.
...
Figure\ \@ref(fig:figCbgResults)B shows the average endorsed prevalence, computed using the procedure by @Cimpian2010 and described in the previous section, in comparison to the average implied prevalence from the same items (Ch. 2 Expt 1b). 
We see that any differences that may exist for truth judgments at the various prevalence levels disappears when the *average endorsed prevalence* is computed. 
This signature, combined with the very property-sensitive *implied prevalence*, produces asymmetries to varying degrees. 
...
```{r cbgModelResults, fig.width=10.5, fig.asp = 0.35, fig.cap="Model predictions for Expt. 1."}
...
However, the model does a poor job at predicting the endorsement data for the generics about accidental properties. 
This is a direct result of the prevalence priors for accidental generics, which skew the probability mass towards low prevalence levels, analagous to the *rare weak* priors in Figure\ \@ref(fig:asymmetrySimulations).
This suggests something more is at play with the accidental property generics. 
We assume in our simulations, for instance, that the prevalence communicated by the speaker model is the prevalence supplied to participants. 
If, however, participants believe that the prevalence level told to them is unrealisitically high and would predict in the future that the prevalence would be lower (analagous to Ch. 4 Expt. 1), we would expect lower endorsements than the ones shown here.
It is not our goal to explain the fine-grained details of the endorsement data, here, however, only that the average endorsed prevalence is relatively insensitive to the property. 
We leave exploration of this *predictive prevalence* explanation for future work. 
...
The goal is to estimate the variable in average endorsed prevalence for items we know have high variability in average implied prevalence.
...
Our primary analysis concerns comparing the prevalence implied by a novel generic sentence (measured in Ch. 2, Expt. 2) to the average prevalence at which a participant endorses the generic. 
We use our computational model, which has been previously fit to the implied prevalence data (Ch. 2, Expt. 2) earlier to generate predictions for the average endorsement prevalence. 
The computational model predictions are dependent on a softmax parameter, which should be fit to the data.
However, we report *a priori* prediction quantities setting this parameter equal to 5, and note that the rank ordering of these predictions (as opposed to the exact quantitative values) is relatively stable across different parameter settings. 
These exact, quantitative predictions were preregistered.

Because of the mathematical properties of *average endorsed prevalence*, the expected variability across items is very small (all model predictions range between 0.52 and 0.73). 
Therefore, to estimate the theoretical maximum explainable variance $\rho$ in the data set, we compute split-half correlations by repeating splitting the data set in half by participants, computing averaged endorsed prevalence for each item in each split-half, computing correlation between two halves of the data set, and using the Spearman-Brown prediction formula to estimate predicted reliability with full sample size. 
$\rho =$ `r format(df.end.1.atc.splithalf.r.summary[[1, "mean_r"]], digits = 2)`(`r format(df.end.1.atc.splithalf.r.summary[[1, "lower_r"]], digits = 2)`, `r format(df.end.1.atc.splithalf.r.summary[[1, "upper_r"]], digits = 2)`), indicating there is a substantial amount of variability in this derived measured. 
Our model's *a priori* model predictions for the average endorsed prevalence on a by-item basis showed a correlation with the data comparable to that of the explainable variance $r(`r model.cortest[["parameter"]][["df"]]`)$.
The results were extremely comparable using for the more relaxed exlcusion criteria.
