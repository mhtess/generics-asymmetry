...
df.end.1.aep.rdy <- left_join(df.end.1.filtered.property %>%
  ...
  mutate(posteriorProb_unnormalized = mean * normalized_need_prob) %>%
  group_by(property) %>%
  mutate(needPosterior_normalized = posteriorProb_unnormalized / sum(posteriorProb_unnormalized),
         uniformPosterior_normalized = mean / sum(mean)) %>%
  select(-n, -empirical_stat, -ci_lower, -ci_upper, -need_probability)
...
# model prediction computed in asymmetry-models.Rmd (should port over here)
